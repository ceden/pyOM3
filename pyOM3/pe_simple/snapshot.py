...
class snap_nc4(model):
      ...
      def prepare_for_run(self, nf_file = 'snap.cdf' ):
          """
      ...
      def diagnose(self,n,tau):  
          ...
          if n==0 or np__.mod(n,int(self.snapint/self.dt))==0. : # writing to file
             if self.my_pe == 0: 
                 print(" --> writing to file ",self.nf_file)
...
class snap_raw(model):
      ...
      def prepare_for_run(self):
          ...
          if self.my_pe == 0:             
             if os.path.isdir(raw_name):  shutil.rmtree(raw_name, ignore_errors=False)
             os.mkdir(raw_name)              
             for pe in range(self.n_pes_x*self.n_pes_y): os.mkdir(raw_name+'/pe=%i'%pe)  
             OM.np.save(raw_name+'/xt.npy',  OM.np.arange(nx,dtype=OM.prec)*self.dx + self.dx/2.)
             OM.np.save(raw_name+'/yt.npy',  OM.np.arange(ny,dtype=OM.prec)*self.dy + self.dy/2.)
             OM.np.save(raw_name+'/zt.npy',  OM.np.arange(nz,dtype=OM.prec)*self.dz + self.dz/2 )          
             OM.np.save(raw_name+'/xu.npy',  OM.np.arange(nx,dtype=OM.prec)*self.dx + self.dx )
             OM.np.save(raw_name+'/yu.npy',  OM.np.arange(ny,dtype=OM.prec)*self.dy + self.dy )
             OM.np.save(raw_name+'/zu.npy',  OM.np.arange(nz,dtype=OM.prec)*self.dz + self.dz )  
             OM.np.save(raw_name+'/decomposition.npy',  OM.np.array([self.n_pes_x,self.n_pes_y]) )   
          return
      ...
      def diagnose(self,n,tau):  
          ...
          t =  n*self.dt     
          if self.my_pe==0:
             if hasattr(self,'congr_itts'):     print("t=",show_time(t)," n=",n," itts=",self.congr_itts)
             else:                              print("t=",show_time(t)," n=",n)
            
          if n==0 or np__.mod(n,int(self.snapint/self.dt))==0.: # writing to file
             if self.my_pe==0: print(" --> writing to raw output to ",self.raw_name)   
             d =  self.raw_name + '/pe=%i/n=%i/'%(self.my_pe,n)  
             os.mkdir(d)   
